#!/usr/bin/env python3
"""Comparison Bench — head-to-head Mazemaker QA bench against models that
...
-----
    ...
    python -u -m benchmarks.external.comparison_bench --n 20 --enable-colbert
...
def _render_markdown(payload: dict[str, Any], args) -> str:
    ...
    n = ds.get("size", "?")
    lines: list[str] = []
    lines.append(f"# Comparison Bench — {payload['timestamp']}")
    lines.append("")
    lines.append(
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="Comparison Bench (Mazemaker vs Hindsight's failed-models list)")
